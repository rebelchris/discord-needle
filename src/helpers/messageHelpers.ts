...
export async function getThreadStartMessage(
  threadChannel: TextBasedChannels | null
  ...
  return parentChannel.messages.fetch(threadChannel.id).catch(() => {
    console.error(
      `Start message has been deleted in thread "${threadChannel.name}"`
    );
    return null;
  });
}
